...
def generate_all(round_stats: list, experiments: list, output_dir: str):
    os.makedirs(output_dir, exist_ok=True)
    paths = []

    # 1. Score comparison (attack vs verify)
    ...
    ax1.set_title("AutoVoiceEvals: Attack -> Improve -> Verify\n(Red = before, Green = after prompt improvement)",
    ...
    for i, (l, c) in enumerate(zip(labels, csats)):
    ...
    for i, exp in enumerate(experiments):
        ...
        seen.add(d)
        ax.scatter(i + 1, exp["score"], color=tier_colors.get(d, "#999"),
                   s=120, edgecolors="white", lw=1.5, label=label, zorder=5)
        if not exp.get("passed", True):
            ax.scatter(i + 1, exp["score"], color="none", s=200,
                       edgecolors="#e74c3c", lw=2.5, zorder=6)
    ...
    if issue_counts:
        ...
        a1.pie(issue_counts.values(), labels=[k[:25] for k in issue_counts.keys()],
               autopct="%1.0f%%", colors=cols, textprops={"fontsize": 9})
        a1.set_title("Issues by Type", fontsize=13, fontweight="bold")

        sev_c = {"critical": "#8e44ad", "high": "#e74c3c", "medium": "#f39c12", "low": "#2ecc71"}
        sev_labels = [s for s in ["critical", "high", "medium", "low"] if s in sev_counts]
        a2.pie([sev_counts[s] for s in sev_labels],
               labels=[s.upper() for s in sev_labels], autopct="%1.0f%%",
               colors=[sev_c.get(s, "#999") for s in sev_labels], textprops={"fontsize": 10})
